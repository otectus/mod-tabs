...
public class FTBTeamsInspector {
    ...
    public static void inspectModClasses() {
        try {
            ...
            LegendaryTabs.LOGGER.info("=== FTB Teams ModItems Fields ===");
            for (Field field : fields) {
                try {
                    Object fieldValue = field.get(null);
                    LegendaryTabs.LOGGER.info("Field: {} = {} (Type: {})", field.getName(), fieldValue, field.getType().getSimpleName());

                    if (fieldValue != null) {
                        LegendaryTabs.LOGGER.info("  Field value class: {}", fieldValue.getClass().getName());
                        Method[] methods = fieldValue.getClass().getMethods();
                        for (Method method : methods) {
                            if (method.getName().equals("get") && method.getParameterCount() == 0) {
                                LegendaryTabs.LOGGER.info("  Has get() method: {}", method);
                            }
                        }
                    }
                } catch (Exception e) {
                    LegendaryTabs.LOGGER.warn("Failed to inspect field {}: {}", field.getName(), e.getMessage());
                }
            }
        } catch (ClassNotFoundException e) {
            LegendaryTabs.LOGGER.info("ModItems class not found, trying alternative locations");
            ...
            for (String className : possibleClasses) {
                try {
                    ...
                    LegendaryTabs.LOGGER.info("Found alternative items class: {}", className);
                    Field[] fields = itemsClass.getFields();
                    for (Field field : fields) {
                        LegendaryTabs.LOGGER.info("Field in {}: {}", className, field.getName());
                    }
                } catch (ClassNotFoundException ex) {
                    LegendaryTabs.LOGGER.debug("Class {} not found", className);
                }
            }
        } catch (Exception e) {
            LegendaryTabs.LOGGER.error("Failed to inspect FTB Teams classes", e);
        }
    }
    ...
    public static Item tryGetTeamItem() {
            ...
            for (String itemName : possibleItems) {
                try {
                    ...
                    if (item instanceof Item) {
                        ...
                        LegendaryTabs.LOGGER.info("Successfully found FTB Teams item: {} from {}.{}", cachedTeamItem, className, itemName);
                        return cachedTeamItem;
                    }
                } catch (Exception e) {
        ...
        LegendaryTabs.LOGGER.info("No FTB Teams items found via reflection, will use fallback");
