...
public final class GlobalSettingsPanel {
    ...
    private static final Identifier COGWHEEL_TEX =
            Identifier.fromNamespaceAndPath(ModTabs.MOD_ID, "textures/gui/cogwheel.png");

    private GlobalSettingsPanel() {}

    public static Identifier cogwheelTexture() { return COGWHEEL_TEX; }
    ...
    public static void render(GuiGraphicsExtractor gui, Screen screen, int mouseX, int mouseY) {
        ...
        // Header bar
        gui.text(Minecraft.getInstance().font, "Global Settings", wx + GS_PAD, wy + 5, 0xFFCCFFCC, false);
        ...
        for (int i = 0; i < navLabels.length; i++) {
            ...
            gui.text(Minecraft.getInstance().font, navLabels[i], r[0] + 6, r[1] + 5, textColor, false);
        }
    ...
    /**
     * Manual tooltip render — we can't use the screen's tooltip renderer here because that
     * path queues into the active screen's tooltip pass, which silently drops the tooltip
     * when called from inside our modal render (which draws in its own stratum during the
     * host screen's render event). Drawing a styled box ourselves matches the rest of the
     * panel's chrome (dark fill + green border) and guarantees the tooltip appears at the
     * cursor immediately.
     */
    private static void drawIconTooltip(GuiGraphicsExtractor gui, Screen screen, String label, int mouseX, int mouseY) {
        ...
        gui.text(font, label, x + padX, y + padY, 0xFFCCFFCC, false);
    }

    private static void drawFooterButton(GuiGraphicsExtractor gui, int[] r, String label, int bg) {
        int border = 0xFF44FF66;
        ...
        gui.text(Minecraft.getInstance().font, label,
                r[0] + (r[2] - tw) / 2, r[1] + 4, 0xFFCCFFCC, false);
    }

    private static void renderVisibilityTab(GuiGraphicsExtractor gui, Screen screen) {
        int[] cr = gsContentRect(screen);
        int colW = cr[2] / 2;
        int leftColX = cr[0];
        ...
        gui.text(Minecraft.getInstance().font, "Visible", leftColX + GS_PAD, cr[1] + 4, 0xFFAAFFAA, false);
        gui.text(Minecraft.getInstance().font, "Hidden", rightColX + GS_PAD, cr[1] + 4, 0xFFFFAAAA, false);
    ...
    private static void renderStickyTab(GuiGraphicsExtractor gui, Screen screen) {
        int[] cr = gsContentRect(screen);
        int colW = cr[2] / 2;
        int leftColX = cr[0];
        int rightColX = cr[0] + colW;
        ...
        // Headers (drawn outside the scroll area so they stay pinned)
        gui.text(Minecraft.getInstance().font, "Not Sticky", leftColX + GS_PAD, cr[1] + 4, 0xFFAAAAAA, false);
        gui.text(Minecraft.getInstance().font, "Sticky", rightColX + GS_PAD, cr[1] + 4, 0xFFFFDD55, false);
        gui.fill(leftColX, cr[1] + GS_HEADER_H - 4, cr[0] + cr[2], cr[1] + GS_HEADER_H - 3, 0xFF44FF66);
    ...
    private static void renderOrderTab(GuiGraphicsExtractor gui, Screen screen, int mouseX, int mouseY) {
        int[] cr = gsContentRect(screen);
        gui.text(Minecraft.getInstance().font, "Drag to reorder",
    ...
    private static void renderGeneralTab(GuiGraphicsExtractor gui, Screen screen) {
        int[] cr = gsContentRect(screen);
        gui.text(Minecraft.getInstance().font, "Icon offset (pixels)",
                cr[0] + GS_PAD, cr[1] + 4, 0xFFAAAAAA, false);
        gui.fill(cr[0], cr[1] + GS_HEADER_H - 4, cr[0] + cr[2], cr[1] + GS_HEADER_H - 3, 0xFF44FF66);
        ...
        for (int i = 0; i < 4; i++) {
            ...
            gui.text(Minecraft.getInstance().font, labels[i],
        ...
        gui.text(Minecraft.getInstance().font, sectionLabel,
    ...
    private static void gsRenderInput(GuiGraphicsExtractor gui, int[] r, String value, boolean focused) {
        int bg = 0xFF1A1F23;
        ...
        gui.text(Minecraft.getInstance().font, display, textX, r[1] + 3, 0xFFFFFFFF, false);
    }
    ...
    private static void renderTabIconAt(GuiGraphicsExtractor gui, String shortKey, int slotX, int slotY) {
