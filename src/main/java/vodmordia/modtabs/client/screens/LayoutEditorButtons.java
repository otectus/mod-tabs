...
public final class LayoutEditorButtons {
    ...
    public static void addToScreen(Screen screen, java.util.function.Consumer<AbstractWidget> add) {
        ...
        // and re-anchor themselves each frame in their extractContents/isMouseOver overrides.
    ...
    public static class IconRotationCycle extends EditOnly {
        ...
        @Override
        protected void extractContents(@NotNull GuiGraphicsExtractor gui, int mouseX, int mouseY, float partial) {
            if (!TabsMenu.isEditing(getScreenInternal())) return;
            syncPosition();
            this.setMessage(Component.literal(TabsMenu.currentIconRotation() + "°"));
            super.extractContents(gui, mouseX, mouseY, partial);
        }
    ...
    public static class CustomIconDropdown extends EditOnly {
        ...
        private void renderPopup(GuiGraphicsExtractor gui, int mouseX, int mouseY) {
            List<String> files = listIconFiles();
            var font = Minecraft.getInstance().font;
            int x = getX();
            int y = getY() + getHeight();
            ...
            // 26.1: no Z lift here — layering above neighbouring panel widgets is handled
            // by TabsMenu's pass-3 ordering (dropdowns render last among the panel widgets).
            ...
            for (int i = 0; i < total; i++) {
                ...
                gui.text(font, drawn, x + 3, rowY + 2, 0xFFFFFFFF, false);
            }
        }
     ...
     * Custom-rendered folder glyph (no default button body) so it doesn't try to fit a
    ...
    public static class MaxTabsPerPageEditBox extends EditBox {
        ...
        @Override
        public void extractWidgetRenderState(@NotNull GuiGraphicsExtractor gui, int mouseX, int mouseY, float partial) {
            if (!TabsMenu.isEditing(screen)) return;
            syncPosition();
            super.extractWidgetRenderState(gui, mouseX, mouseY, partial);
        }
    ...
    public static class EditToggle extends Button {
        ...
        protected void extractContents(@NotNull GuiGraphicsExtractor gui, int mouseX, int mouseY, float partial) {
            ...
            // Default-looking button body (sprite + centered label), mirroring Button.Plain.
            this.extractDefaultSprite(gui);
    ...
    public static class CogwheelButton extends EditOnly {
        ...
        protected void extractContents(@NotNull GuiGraphicsExtractor gui, int mouseX, int mouseY, float partial) {
            ...
            gui.pose().scale(s, s);
            gui.blit(RenderPipelines.GUI_TEXTURED, GlobalSettingsPanel.cogwheelTexture(), 0, 0, 0f, 0f, 25, 25, 25, 25);
            gui.pose().popMatrix();
        }
    }
